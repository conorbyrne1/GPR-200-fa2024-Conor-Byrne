...
int main() {
	...
	for (int i = 0; i < 100; i++)
	{
		...
		cubePositions[i] = glm::vec3((rand() % 51 - 25), (rand() % 51 - 25), (rand() % 51 - 25));
	}
	...
	//COLOR RGBA
	glVertexAttribPointer(1, 4, GL_FLOAT, GL_FALSE, 5 * sizeof(float), (void*)(sizeof(float) * 3));
	...
	while (!glfwWindowShouldClose(window)) {
		...
		for (unsigned int i = 0; i < 100; i++)
		{
			...
			model += scale(i * sin(time), i * sin(time), i * sin(time)) * model;
